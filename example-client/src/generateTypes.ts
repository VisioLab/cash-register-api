...
const toMessageTypeName = (id: string): string => `${id.charAt(0).toUpperCase()}${id.slice(1)}`
...
const messageTypes = await Promise.all(
  document
    ...
    .all()
    .map((message) =>
      compile(message.payload()?.json() ?? {}, toMessageTypeName(message.id()), {
        bannerComment: "",
...
const schemaTypes = await Promise.all(
  document
    ...
    .map((schema) => compile(schema.json(), schema.id(), {bannerComment: "", additionalProperties: false})),
)
