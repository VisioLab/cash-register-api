...
export class CashRegister {
  ...
  async paymentSuccess() {
    const message: PaymentSuccess = {
      ...
      data: {
        ...
        remainingBalance: 4.9,
        remainingAllowance: 1.2,
      },
    }
  ...
  async onStartPayment(message: StartPayment) {
    ...
    if (!paymentInProgress && !_.isEmpty(basket)) {
      useCashRegisterStore.setState({
        ...
        qrCodeContent: message.data.identifier,
      })
    }
  }
  ...
  async onUserInput(message: UserInput) {
    if (message.data.id === "survey") {
      useCashRegisterStore.setState({
        surveyResult: useCashRegisterStore.getState().surveyResult + parseInt(message.data.action),
      })
    }
  }
  ...
  private async receiveMessage(message: MessageEvent<string>) {
    ...
    switch (parsedMessage.event) {
      ...
      default:
        await this.send({
          event: "error",
          data: {reason: "unexpectedEvent", message: `Received unexpected message ${JSON.stringify(parsedMessage)}`},
        })
    }
  }
